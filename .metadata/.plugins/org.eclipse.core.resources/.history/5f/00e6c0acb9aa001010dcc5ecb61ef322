/*
 * myApp_PCcom.h
 *
 *  Created on: Oct 8, 2025
 *      Author: Paulo
 */

#ifndef API_INC_MYAPP_PCCOM_H_
#define API_INC_MYAPP_PCCOM_H_

#ifdef __cplusplus
extern "C" {
#endif

#include <stdint.h>
#include <ctype.h>
#include <stdbool.h>
#include <string.h>
#include <stdio.h>
#include "stm32f4xx_hal.h"
#include "usart.h"

#define MAX_LEN_STRING 100

typedef enum {
	UART_ERROR_OK,
	UART_ERROR_WAIT_COMMAND,
	UART_ERROR_STRING_NULL,
	UART_ERROR_SIZE_ZERO,
	UART_ERROR_BUSY,
	UART_ERROR_RECEIVE,

	UART_ERROR_COUNT
} _uartErrors;

typedef enum {
	PC_COMMAND_NONE,
	PC_COMMAND_RST,
	PC_COMMAND_STATUS,
	PC_COMMAND_THRESHOLD,
	PC_COMMAND_UNKNOWN_COMMAND,

	PC_COMMAND_COUNT
} _commandPC;

uint8_t* getParameter(void);
/*
 *@brief uartInit: UART initialization.
 *@param None.
 *@return Status of initialization. True if success, else false.
 */
bool uartInit(void);

/*
 * @brief uartSendString: Receives a pointer to a string that you want to send over the UART.
 * @param uint8_t * pstring: A pointer to a string that you want to send.
 * @return None.
 * */
void uartSendString(uint8_t * pstring);

/*
 * @brief uartSendStringSize: Receives a pointer to a string to be sent over the UART. Also, the number of characters to be sent.
 * @param uint8_t * pstring: A pointer to a string that you want to send.
 * @param uint16_t size: The number of characters to be sent.
 * @return None.
 * */
void uartSendStringSize(uint8_t * pstring, uint16_t size);

_commandPC getCommand(void);

//_uartErrors initReceive(void);

//size_t uartGetData(uint8_t * pstring, uint16_t lenPstring, _uartErrors* error);

#ifdef __cplusplus
}
#endif

#endif /* API_INC_MYAPP_PCCOM_H_ */
