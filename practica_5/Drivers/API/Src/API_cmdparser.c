...
/*
 * @brief send_response: Helper function to send text responses via safe_send, ensuring that they always end in \r\n
...
/*
 * @brief reset_line: Write the initial parameters of the MEF
 * @return None
 * */
...
/*
 *@brief tokenizeLine: Converts a line of text into a set of words separated by spaces/tabs, and returns them as an array of strings without using extra memory (modifies the original string by inserting \0).
...
/*
 * @brief parseCommand: Takes the tokens extracted from a received line and decides what command it is, whether the syntax is valid, and what action (cmd_action_t) should be executed.
 * @param char* tokens[]: Array of pointers where the beginning of each token is stored.
 ...
 * */
static cmd_status_t parseCommand(char *tokens[], size_t ntok, cmd_action_t *act) {
...
/*
 *@brief execCommand: Send the command to be executed to the callback.
 *@param const cmd_action_t *act: a pointer to the action queue to be excecuted.
 *@return None.
 * */
...
/*
 * @brief handleReceivedChar: Is the full-line handler in the command parser:
		  - Takes the received text (line_buf).
		  - Separates it into tokens.
		  - Validates that something is present.
		  - Converts the command to uppercase.
		  - Interprets and translates the command (parseCommand).
		  - Executes the action or returns an error message.
...
/*
 * @brief handleReceivedChar: Converts a stream of single characters into valid complete lines, ready to be parsed as commands.
 * @param char c: the stream character.
 * @return None.
 * */
